...
import {pagarFuncion} from "../../service/FuncionServiceImpl";
...
const PagoCompra = () => {
    ...
    const [mediosPago, setMediosPago] = React.useState<MedioPago[]>([]);
    const [estado, setEstado] = React.useState<boolean>(false);
    const [componente, setComponente] = React.useState<JSX.Element>(<Fragment/>);
    ...
    const ubicacion: UbicacionCompra = JSON.parse(window.sessionStorage.getItem('ubicacion_compra') || '{}');
    const usuario: Usuario = JSON.parse(window.localStorage.getItem('usuario') || '{}');

    const select: any = useRef<any>();

    const listaMedioPagoPorPaisQuery = async () => {
        await listaMedioPagoPorPais(funcion.multiplexPelicula.multiplex.ciudad.pais).then((response: any) => {
            let data: MedioPago[] = response.data;
            setMediosPago(data);
        }).catch((error: any) => {
    ...
    const pagoFuncionQuery = async (pagoFuncion: PagoFuncion) => {
        await pagarFuncion(pagoFuncion).then((response: any) => {
            setLoading(false);
            if (response.status === 200) {
                const usuario: Usuario = JSON.parse(window.localStorage.getItem('usuario') || '{}');
                usuario.pt = Number.parseInt(String(usuario.pt)) + Number.parseInt(response.data);
                window.localStorage.setItem('usuario', JSON.stringify(usuario));
                setComponente(<MensajeOk/>)
                setEstado(true);
            } else {
                setComponente(<MensajeError exit={() => setEstado(false)}/>)
                setEstado(true);
            }
        }).catch((error: any) => {
            setLoading(false);
    ...
    const mapMedioPago = (mediosPago: MedioPago[]) => {
        return mediosPago.map((item: MedioPago) => {
            return <option key={item.id} value={JSON.stringify(item)}>{item.nombre}</option>
        });
    }

    const pagar = (e: any) => {
        e.preventDefault();
        const pagarFuncion: PagoFuncion = {
            idFuncion: funcion.id,
            sillas: ubicacion.sillas,
            cantidadSillas: localidad.cantidad,
            // @ts-ignore
            idUsuario: usuario.id,
            tipoLocalidad: localidad.tipoLocalidad,
            medioPago: JSON.parse(select.current.value)
        }
        setLoading(true);
        pagoFuncionQuery(pagarFuncion).then(() => null);
    }

    useEffect(() => {
        listaMedioPagoPorPaisQuery().then(() => null);
    }, []);
    ...
    return (
        <Fragment>
            <div className="md:w-1/2 w-full mx-auto">
                <div className="text-center md:m-8 bg-white p-8 md:rounded shadow-lg">
                    <div className="mt-4 p-4">
                        <h1 className="text-xl font-semibold text-gray-700 text-center">{menuTranslation('pago.title')}</h1>
                        <form onSubmit={(e) => pagar(e)} className="">
                            <div className="my-3">
                                <input
                                    required={true}
                                    type="text"
                                    className="block w-full px-5 py-2 border rounded bg-white shadow-lg placeholder-gray-400 text-gray-700 focus:ring focus:outline-none"
                                    placeholder={menuTranslation('pago.titular')}
                                />
                            </div>
                            <div className="my-3">
                                <input
                                    ...
                                    type="number"
                                    className="block w-full px-5 py-2 border rounded bg-white shadow-lg placeholder-gray-400 text-gray-700 focus:ring focus:outline-none"
                                    placeholder={menuTranslation('pago.numero_tarjeta')}
                                />
                            </div>
                            <div className="my-3 flex flex-col">
                                <div className="mb-2">
                                    <label className="text-gray-700">{menuTranslation('pago.Venciomento')}</label>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                    <select
                                        required={true}
                                        className="form-select appearance-none block w-full px-5 py-2 border rounded bg-white shadow-lg placeholder-gray-400 text-gray-700 focus:ring focus:outline-none"
                                    >
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min={0}
                                        className="block w-full col-span-2 px-5 py-2 border rounded bg-white shadow-lg placeholder-gray-400 text-gray-700 focus:ring focus:outline-none"
                                        placeholder={menuTranslation('pago.codigo_seguridad')}
                                    />
                                </div>
                                <div className="my-3 flex flex-col">
                                    <div className="mb-2">
                                        <label className="text-gray-700">{menuTranslation('menu.medio_pago')}</label>
                                        <div className="flex mt-2 mx-4">
                                                            <span>
                                                            <svg className="mr-2 -ml-1 w-7 h-4" viewBox="0 0 601 360"
                                                                 fill="none"
                                            ...
                                            <span><svg className="mr-2 -ml-1 w-10 h-3" viewBox="0 0 660 203" fill="none"
                                                       xmlns="http://www.w3.org/2000/svg">
                                                                <path
                                    ...
                                    <select required={true}
                                        ...
                                        <option value={""} selected
                                                disabled>{commonTranslation("select_index")}</option>
                                        {mapMedioPago(mediosPago)}
                                    </select>
                                </div>
                            </div>
                            <div className="mt-6 p-4">
                                <button
                                    type="submit"
                                    className="w-full px-5 py-2 font-semibold text-white bg-gray-800 rounded-lg shadow-md hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-80"
                                >
                                    {menuTranslation('pago.pagar')}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <Modal estado={estado} exit={() => setEstado(false)} opcional={false} component={componente}/>
            <LoadFullV2 estado={loading}/>
        </Fragment>
    )
}
